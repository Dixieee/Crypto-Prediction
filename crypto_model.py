...
# Menonaktifkan RuntimeWarning
warnings.filterwarnings('ignore', category=RuntimeWarning)

# Inisialisasi KuCoin Exchange
exchange = ccxt.kucoin()

@st.cache_data(ttl=86400)  # Cache data yang diperbarui setiap 1 hari
def fetch_top_crypto():
    markets = exchange.fetch_markets()
    data = []
    
    # Hanya memilih pasangan mata uang yang aktif dan berakhir dengan '/USDT'
    usdt_markets = [market for market in markets if market['symbol'].endswith('/USDT') and market['active']]
    
    for market in usdt_markets:
        # Ambil ticker tanpa try-except untuk pasangan
    ...
    if not data:
        st.error("Tidak ada data yang berhasil diambil dari KuCoin.")
...
def analyze(symbol, n_days):
    try:
        status_placeholder = st.empty()  # Placeholder untuk status loading
        ...
        future_prices = predicted_prices_denorm[-365:]
        future_dates = pd.date_range(start=df['timestamp'].iloc[-1], periods=n_days + 1, freq='D')

        # Visualisasi dengan Plotly Interactive Chart
